...
def _data_transforms_cifar100(cutout_size, autoaugment=False):
    ...
    #  1
        train_transform = transforms.Compose([
            transforms.RandomCrop(32, padding=4),
            transforms.RandomHorizontalFlip(),
            CIFAR10Policy(),	#这里先借用了CIFAR10的数据增强AutoAugment方法
            transforms.ToTensor(),
            transforms.Normalize(CIFAR_MEAN, CIFAR_STD),
        ])
...
def build_train_cifar10(args, cutout_size=None, autoaugment=False):
    ...
    train_data = dataset.CIFAR10(root=args.data, train=True, download=False, transform=train_transform)
    valid_data = dataset.CIFAR10(root=args.data, train=False, download=False, transform=valid_transform)
...
def build_search_cifar100(args, ratio=0.9,cutout_size=None, autoaugment=False,num_workers = 10):
    ...
    train_transform, valid_transform = _data_transforms_cifar100(cutout_size,autoaugment)

    train_data = dataset.CIFAR100(root=args.data, train=True, download=False, transform=train_transform)
    valid_data = dataset.CIFAR100(root=args.data, train=True, download=False, transform=valid_transform)

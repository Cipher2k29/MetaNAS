...
from model.network import MLRSNet
...
parser = argparse.ArgumentParser(description='for mlr-snet')
parser.add_argument('-device', type=str, default='cuda')
parser.add_argument('-dataset', type=str, default='cifar10', choices=['cifar10, cifar100'])  # need adjust, 10 in cifar10, 100 in cifar100
args = parser.parse_args()

mlr_snet = MLRSNet(1, 50).to(args.device)
if args.dataset == 'cifar10':
    net_path = './model/Meta_LR_C10.pth'  
elif args.dataset == 'cifar100':
    net_path = './model/Meta_LR_C100.pth'  

model_dict = torch.load(net_path)  
mlr_snet.load_state_dict(model_dict)  
mlr_snet = mlr_snet.to(args.device)  
...
def Model_train(train_queue, model, train_criterion, optimizer, scheduler, args, valid_queue, eval_criterion,
    ...
    writer = SummaryWriter('./logs/search/240910')
    ...
    for epoch in range(args.search_epochs):
        ...
        writer.add_scalar('train_loss',objs.avg, epoch+1)
        writer.add_scalar('train_acc',top1.avg, epoch+1)
        scheduler.step()
        ...
        if print_ or (epoch + 1) == args.search_epochs:
            ...
            print('train accuracy top1:{0:.3f}, train accuracy top5:{1:.3f}, train loss:{2:.5f}'.format(top1.avg,
                                                                                                        top5.avg,
                                                                                                        objs.avg))
            logging.info('train accuracy top1:{0:.3f}, train accuracy top5:{1:.3f}, train loss:{2:.5f}'.format(top1.avg,
                                                                                                               top5.avg,
                                                                                                               objs.avg))
...
def Model_valid(valid_queue, model, eval_criterion, args, count=1):
    ...
    print('valid accuracy top1:{0:.3f}, valid accuracy top5:{1:.3f}, valid loss:{2:.5f}'.format(top1.avg, top5.avg,
                                                                                                objs.avg))
    logging.info(
        'valid accuracy top1:{0:.3f}, valid accuracy top5:{1:.3f}, valid loss:{2:.5f}'.format(top1.avg, top5.avg,
                                                                                              objs.avg))

    writer.add_scalar('test_loss',objs.avg, count)
    writer.add_scalar('test_acc',top1.avg, count)
    count += 1
...
def solution_evaluation(model, train_queue, valid_queue, args):
    ...
    result = Model_train(train_queue, model, train_criterion, optimizer, scheduler, args, valid_queue, eval_criterion,
                         print_=True)  # True for DEBUG

    Flops = Calculate_flops(model)
    # print('result:',result,'\n',1-result[3]/100,'\n', result[6]) # result[3] means valid_top1_acc
    # print('params and flops:',num_parameters, Flops)
...
def Model_Evaluation(args):
    ...
    if args.classes == 10:
        train_queue, valid_queue = build_search_cifar10(args=args, ratio=0.9,
                                                    num_workers=args.search_num_work)  # change dataset
    elif args.classes == 100:
        train_queue, valid_queue = build_search_cifar100(args=args, ratio=0.9,
                                                    num_workers=args.search_num_work)  # change dataset
    # ============================================ build model based on corresponding dag ============================================	#change classes
    model = NetworkCIFAR(args, args.classes, args.search_layers, args.search_channels, DAG_, args.search_use_aux_head,
    ...
    logging.info(
        'Final, total_used time:{0:.3f} min, valid_top1_acc:{1:.3f}%, valid_top5_acc:{2:.3f}%, Parameters:{3:.4f}MB, Flops:{4:.4f}MB'
